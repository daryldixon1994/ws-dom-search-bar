...
function handleSearch() {
  ...
  if (search !== "") {
    let result = [];
    moviesList.setAttribute("id", "list");

    for (const elt of movies) {
      if (elt.title.toLowerCase().includes(search.toLowerCase())) {
        result.push(elt);
        console.log("result :", result);
      }
    }
    moviesList.innerHTML = result.map((elt) => {
      return `
        ...
        <div class="movie-item-details">
          ...
          <h5>Romance, Drama</h5>
          <h6>${elt.year}</h6>
          <button type="button" class="search-btn">Watch</button>
        </div>
      </div>
      `;
    });
